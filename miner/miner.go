	...
	for {
		...
		case ev := <-dlEventCh:
			...
			case downloader.StartEvent:
				log.Debug("Miner: syncing, stopping mining")
				...
				if c, ok := miner.worker.engine.(*co2.Co2); ok {
					c.SetSyncing(true)
				}
		...
		case <-miner.startCh:
			...
			shouldStart = true
		case <-miner.stopCh:
			log.Debug("Miner: got a message on the Stop Channel stopping mining")
			shouldStart = false
			miner.worker.stop()
		case <-miner.exitCh:
			log.Debug("Miner: got a message on the Exit Channel closing mining")
			miner.worker.close()
			return
		}
	}
