...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, genesis *Genesis, overrides *ChainOverrides, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64) (*BlockChain, error) {
	...
	// Save the head in case of a rewind
	if c, ok := bc.engine.(*co2.Co2); ok {
		c.SetLatestChainHeader(bc.CurrentHeader())
	}
	...
	if co2Engine, ok := engine.(*co2.Co2); ok && head.Number.Uint64() > 0 && co2Engine.IsExecutor() {
		if co2Engine.IsHeaderSignedBySequencer(head) {
			log.Debug("Header is signed by sequencer, sending to sequencer engine", "number", head.Number, "hash", head.Hash())
			block := bc.GetBlockByHash(head.Hash())
			if block != nil {
				co2Engine.GetSequencerBlockChannel() <- block
			} else {
				log.Warn("Head block not found", "number", head.Number, "hash", head.Hash())
			}
		}
	}
	return bc, nil
...
func (bc *BlockChain) InsertChain(chain types.Blocks) (int, error) {
	if c, ok := bc.engine.(*co2.Co2); ok {
		...
		for i, block := range chain {
			log.Debug("Got a new block", "number", block.Number(), "hash", block.Hash(), "difficulty", block.Difficulty())
			startTime := time.Now()
			if i == 0 {
				currentBlock = bc.CurrentHeader()
				ltst := c.GetLatestChainHeader()
				if ltst != nil && ltst.Number.Uint64() > currentBlock.Number.Uint64() {
					currentBlock = ltst
				}
			} else {
			...
			if err := c.ValidateBlockForInsertion(block, currentBlock, bc.GetBlock); err != nil {
				...
				log.Error("Rejecting block", "blockNumber", block.Number(), "err", err)
				return i, err
			} else {
				log.Info("Accepting block", "number", block.Number(), "difficulty", block.Difficulty(), "hash", block.Hash())
				if c.IsExecutor() {
					ok, prevState := c.NextExecutionStep(co2.InsertingSequencerBlock)
					if !ok {
						log.Warn("Skipping Exeution Step", "current state", c.GetExecutionState(), "previous state", prevState)
					}
				}
			}
			endTime := time.Now()
			log.Info("Block validation time", "blockNumber", block.Number(), "time", endTime.Sub(startTime))
		}
	}
...
func (bc *BlockChain) insertChain(chain types.Blocks, setHead bool) (int, error) {
	...
	for ; block != nil && err == nil || errors.Is(err, ErrKnownBlock); block, err = it.next() {
		log.Debug("Processing block", "number", block.Number(), "hash", block.Hash(), "num txs",
			len(block.Transactions()), "gas", block.GasUsed(), "root", block.Root(), "difficulty", block.Difficulty())
		startTime := time.Now()
		...
		if c, ok := bc.engine.(*co2.Co2); ok {
			if c.IsExecutor() && c.IsHeaderSignedBySequencer(block.Header()) {
				triggerCh := c.GetSequencerBlockChannel()
				log.Debug("Sending the red block to the trigger channel", "blocks in channel", len(triggerCh))
				triggerCh <- block
				log.Debug("Sent the red block to the trigger channel", "blocks in channel", len(triggerCh))
				endTime := time.Now()
				log.Debug("Red Block processing time", "number", block.Number(), "hash", block.Hash(), "duration", endTime.Sub(startTime))
				return it.index, nil
			}
		}
		...
		endTime := time.Now()
		log.Debug("Black Block processing time", "blockNumber", block.Number(), "hash", block.Hash(), "duration", endTime.Sub(startTime))
	}
	...
	if c, ok := bc.engine.(*co2.Co2); ok && lastCanon != nil {
		if c.IsExecutor() && c.IsHeaderSignedBySequencer(lastCanon.Header()) {
			log.Debug("Re-inserted the last red block, sending it to the trigger channel",
				"number", lastCanon.Number(), "hash", lastCanon.Hash())
			triggerCh := c.GetSequencerBlockChannel()
			triggerCh <- lastCanon
			return it.index, nil
		}
...
func (bc *BlockChain) RemoveBlock(hash common.Hash, num uint64) {
	batch := bc.db.NewBatch()
	rawdb.DeleteBlock(batch, hash, num)
	if err := batch.Write(); err != nil {
		log.Error("RemoveBlock failed", "err", err)
	}
	// We also remove the block from the cache
	if ok := bc.blockCache.Remove(hash); !ok {
		log.Error("Failed to remove block from cache", "hash", hash)
	}
}
