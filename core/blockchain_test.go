...
func testCreateThenDelete(t *testing.T, config *params.ChainConfig) {
	...
	_, blocks, _ := GenerateChainWithGenesis(gspec, engine, 2, func(i int, b *BlockGen) {
		...
		nonce++
		nonce++ // we need to increment the nonce twice, as the contract creation will deploy a secondary contract
		b.AddTx(tx)
		tx, _ = types.SignNewTx(key, signer, &types.LegacyTx{
			Nonce:    nonce,
			GasPrice: new(big.Int).Set(fee),
...
func TestTransientStorageReset(t *testing.T) {
	...
	_, blocks, _ := GenerateChainWithGenesis(gspec, engine, 1, func(i int, b *BlockGen) {
		...
		nonce++
		// we need to increment the nonce twice,
		//as the contract creation will deploy a secondary contract
		nonce++
