...
// EVMAwarePrecompiledContract is an interface designed for precompiled contracts requiring EVM access.
// Precompiled contracts that need to interact with the EVM should implement this interface,
// and include the EVMAwareRun method to establish the EVM and other parameters in the EVMAwareRun
...
var PrecompiledContractsHomestead = map[common.Address]PrecompiledContract{
	...
	// Add mpc precompiled contracts
	common.BytesToAddress([]byte{100}): &mpcContract{},
}
...
func RunPrecompiledContract(p PrecompiledContract, evm *EVM, caller common.Address, addr common.Address, input []byte, suppliedGas uint64) (ret []byte, remainingGas uint64, err error) {
	...
	if err != nil {
		log.Error("Failed to run precompiled contract", "address", addr, "error", err)
		if _, ok := MPCTxReversionErrors[err]; ok {
			err = ErrExecutionReverted
			log.Debug("Changed error to ErrExecutionReverted", "error", err)
		}
	}
