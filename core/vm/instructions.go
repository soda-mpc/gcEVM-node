...
func opSload(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	// Load to secured memory a cipher-text whose location is valid in storage
	if err := insertToAuthenticatedMemoryFromStorage(hash, val, interpreter, scope.Contract.Address()); err != nil {
		return nil, err
	}
...
func opSstore(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	validateAndStoreCt(locHash, val.Bytes32(), interpreter, scope.Contract.Address())
...
func opSelfdestruct(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	securedStorage := crypto.CreateAddress(scope.Contract.Address(), 0)
	interpreter.evm.StateDB.SelfDestruct(securedStorage)
