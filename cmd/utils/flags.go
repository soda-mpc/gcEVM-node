...
var (
	// Soda settings
	RunSodaEngineFlag = &cli.BoolFlag{
		Name:     "soda.engine",
		Usage:    "Sets the system consensus engine to Soda",
		Category: flags.SodaCategory,
	}
	SodaRoleSequencerFlag = &cli.BoolFlag{
		Name:     "soda.sequencer",
		Usage:    "The designated instance type is Sequencer",
		Category: flags.SodaCategory,
	}
	SodaRoleExecutorFlag = &cli.BoolFlag{
		Name:     "soda.executor",
		Usage:    "The designated instance type is Executer",
		Category: flags.SodaCategory,
	}
	SodaRoleValidatorFlag = &cli.BoolFlag{
		Name:     "soda.validator",
		Usage:    "The designated instance type is Validator",
		Category: flags.SodaCategory,
	}
	SodaGlobalExecutor1AddressFlag = &cli.StringFlag{
		Name:     "soda.exec1addr",
		Usage:    "The 1st executor address to validate signed execution blocks with",
		Category: flags.SodaCategory,
	}
	SodaGlobalExecutor2AddressFlag = &cli.StringFlag{
		Name:     "soda.exec2addr",
		Usage:    "The 2nd executor address to validate signed execution blocks with",
		Category: flags.SodaCategory,
	}
	SodaGlobalExecutorPeerAddressFlag = &cli.StringFlag{
		Name:     "soda.execpeeraddr",
		Usage:    "The executor's peer address to validate signed execution blocks with",
		Category: flags.SodaCategory,
	}
	SodaGlobalSequencerPubKeyFlag = &cli.StringFlag{
		Name:     "soda.seqpubkey",
		Usage:    "The sequencer public key to identify the sequencer peer",
		Category: flags.SodaCategory,
	}
	SodaGlobalSequencerAddressFlag = &cli.StringFlag{
		Name:     "soda.seqaddr",
		Usage:    "The sequencer address to validate signed transaction blocks with",
		Category: flags.SodaCategory,
	}
	SodaMPCServerIPFlag = &cli.StringFlag{
		Name:     "soda.mpcserverip",
		Usage:    "The IP address of the MPC server",
		Category: flags.SodaCategory,
	}
	SodaMPCServerPortFlag = &cli.StringFlag{
		Name:     "soda.mpcserverport",
		Usage:    "The port of the MPC server",
		Category: flags.SodaCategory,
	}
...
func setSodaConfig(ctx *cli.Context) {
	if ctx.IsSet(RunSodaEngineFlag.Name) {
		mustSetSodaNetRoles(ctx)
		common.SodaEngine = true
	}

}

// mustSetSodaNetRoles creates the node role from CLI flags.
func mustSetSodaNetRoles(ctx *cli.Context) {
	log.Debug("mustSetSodaNetRoles")
	CheckExclusive(ctx, SodaRoleSequencerFlag, SodaRoleExecutorFlag)
	switch {
	case ctx.Bool(SodaRoleSequencerFlag.Name):
		if !ctx.IsSet(SodaGlobalExecutor1AddressFlag.Name) || !ctx.IsSet(SodaGlobalExecutor2AddressFlag.Name) {
			panic("Not all Executor addresses specified!")
		}
		common.SodaExecutor1Address = common.HexToAddress(ctx.String(SodaGlobalExecutor1AddressFlag.Name))
		common.SodaExecutor2Address = common.HexToAddress(ctx.String(SodaGlobalExecutor2AddressFlag.Name))
		common.SodaSequencerAddress = common.HexToAddress(ctx.String(MinerEtherbaseFlag.Name))

		common.SodaRole = common.Sequencer
	case ctx.Bool(SodaRoleExecutorFlag.Name):
		...
		if !ctx.IsSet(MinerEtherbaseFlag.Name) {
			panic("Miner etherbase not specified!")
		}

		if !ctx.IsSet(SodaMPCServerIPFlag.Name) {
			panic("MPC server ip not specified!")
		}

		if !ctx.IsSet(SodaMPCServerPortFlag.Name) {
			panic("MPC server port not specified!")
		}

		common.SodaSequencerAddress = common.HexToAddress(ctx.String(SodaGlobalSequencerAddressFlag.Name))
		common.SodaExecutor1Address = common.HexToAddress(ctx.String(MinerEtherbaseFlag.Name))
		common.SodaExecutor2Address = common.HexToAddress(ctx.String(SodaGlobalExecutorPeerAddressFlag.Name))
		common.SodaRole = common.Executor
		common.MpcServerIP = ctx.String(SodaMPCServerIPFlag.Name)
		common.MpcServerPort = ctx.String(SodaMPCServerPortFlag.Name)
	case ctx.Bool(SodaRoleValidatorFlag.Name):
		if !ctx.IsSet(SodaGlobalExecutor1AddressFlag.Name) ||
			!ctx.IsSet(SodaGlobalExecutor2AddressFlag.Name) ||
			!ctx.IsSet(SodaGlobalSequencerAddressFlag.Name) {
			panic("Not all role addresses specified!")
		}
		if !ctx.IsSet(SodaGlobalSequencerPubKeyFlag.Name) {
			log.Debug("Sequencer public key not specified, this is an external validator.")
			common.SodaSequencerPubKey = "0"
		} else {
			common.SodaSequencerPubKey = ctx.String(SodaGlobalSequencerPubKeyFlag.Name)
		}
		common.SodaSequencerAddress = common.HexToAddress(ctx.String(SodaGlobalSequencerAddressFlag.Name))
		common.SodaExecutor1Address = common.HexToAddress(ctx.String(SodaGlobalExecutor1AddressFlag.Name))
		common.SodaExecutor2Address = common.HexToAddress(ctx.String(SodaGlobalExecutor2AddressFlag.Name))

		common.SodaRole = common.Validator

	default:
		// Node was executed but not properly confihured
		if ctx.IsSet(NetworkIdFlag.Name) {
			panic("No Soda instance type specified!")
		}
		// geth was called without instansiating a node
		log.Debug("No Soda instance type specified!")
	}
}
...
func setEtherbase(ctx *cli.Context, cfg *ethconfig.Config) {
	...
	common.MyAddress = cfg.Miner.Etherbase
}
