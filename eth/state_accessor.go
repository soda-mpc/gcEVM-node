...
func (eth *Ethereum) hashState(ctx context.Context, block *types.Block, reexec uint64, base *state.StateDB, readOnly bool, preferDisk bool) (statedb *state.StateDB, release tracers.StateReleaseFunc, err error) {
	...
	for current.NumberU64() < origin {
		...
		vmCfg := vm.Config{}
		if c, ok := eth.blockchain.Engine().(*co2.Co2); ok {
			c.ApplyInsertBlockConfiguration(block, &vmCfg)
		}
		_, _, _, err := eth.blockchain.Processor().Process(current, statedb, vmCfg)
		if err != nil {
...
func (eth *Ethereum) stateAtTransaction(ctx context.Context, block *types.Block, txIndex int, reexec uint64, vmConfig *vm.Config) (*core.Message, vm.BlockContext, *state.StateDB, tracers.StateReleaseFunc, error) {
