...
func newHandler(config *handlerConfig) (*handler, error) {
	...
	var addTxs func([]*types.Transaction) []error
	if c, ok := config.Chain.Engine().(*co2.Co2); ok && c.IsExecutor() {
		// Executors do not accept transactions
		addTxs = func(txs []*types.Transaction) []error {
			return []error{fmt.Errorf("executors do not accept transactions")}
		}
	} else {
		addTxs = func(txs []*types.Transaction) []error {
	...
	if c, ok := config.Chain.Engine().(*co2.Co2); ok && c.IsExecutor() {
		c.RegisterSigBroadcastFunc(h.broadcastExecutorSig)
		c.RegisterSigRequestFunc(h.requestExecutorSig)
		c.RegisterGetMPCStatusFunc(h.getMPCStatus)
		c.RegisterGetLatestBlockFunc(h.chain.GetBlockByNumber)
		c.RegisterRequestSequencerBlockFunc(h.requestSequencerBlock)
		c.RegisterTriggerSyncFunc(h.chainSync.TriggerSync)
		h.downloader.RegisterNotifySyncEndFunc(c.SyncEnded)
	}
	return h, nil
}
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	log.Debug("Running eth peer", "peer ID", peer.ID(), "peer name", peer.Fullname(), "peer remote addr", peer.RemoteAddr())
	if !h.incHandlers() {
	...
	log.Debug("Propagating existing transactions", "count", len(h.txpool.Pending(false)))
...
func (h *handler) removePeer(id string) {
	if _, ok := h.chain.Engine().(*co2.Co2); ok {
		// In our current set up (which includes 2 Executors and 1 sequencer)
		// disconnecting the executors or the sequencer from a peer breaks the entire protocol.
		// When we introduce decentralization to the protocol, we should allow
		// peers to disconnect from each other.
		// The only role that can disconnect from a peer is the validator, that can disconnect from another validator or sequencer.
		if !h.chain.Engine().(*co2.Co2).IsValidator() {
			log.Error("Peer would have been disconnected", "peer", id)
			return
		}
	}
...
func (h *handler) broadcastExecutorSig(sigDetails *types.ExecutorSigDetails) {
	for _, peer := range h.peers.peers {
		// Find a way to make sure the peer is the executor peer
		peer.AsyncSendExecutorSignature(sigDetails)
	}
}

func (h *handler) requestExecutorSig(blockNum uint64) {
	for _, peer := range h.peers.peers {
		// Find a way to make sure the peer is the executor peer
		peer.AsyncRequestExecutorSignature(blockNum)
	}
}

func (h *handler) requestSequencerBlock(blockNum uint64) {
	for _, peer := range h.peers.peers {
		peer.RequestSequencerBlock(blockNum)
	}
}

func (h *handler) getMPCStatus() *types.MPCStatus {
	return vm.GetMPCStatus()
}
...
func (h *handler) BroadcastBlock(block *types.Block, propagate bool) {
	...
	if h.chain.HasBlock(hash, block.NumberU64()) {
		for _, peer := range peers {
			// Validators should not announce blocks to the sequencer
			if !common.IsExternalValidator() {
				if !common.IsSequencerPubKey(hex.EncodeToString(crypto.FromECDSAPub(peer.Node().Pubkey())[1:])) {
					peer.AsyncSendNewBlockHash(block)
				} else {
					log.Debug("Internal validator not announcing block to sequencer", "peer", peer.ID())
				}
			} else {
				peer.AsyncSendNewBlockHash(block)
			}
