			...
			for peer, hashes := range request {
				...
				go func(peer string, hashes []common.Hash) {
					...
					case res := <-resCh:
						...
						txs, uncles, _, _, _ := res.Res.(*eth.BlockBodiesResponse).Unpack()
		...
		case filter := <-f.headerFilter:
			...
			for _, header := range task.headers {
				...
				if announce := f.fetching[hash]; announce != nil && announce.origin == task.peer && f.fetched[hash] == nil && f.completing[hash] == nil && f.queued[hash] == nil {
					...
					if f.getBlock(hash) == nil && (!common.IsValidator() || header.Difficulty.Cmp(co2.GetDifficulty(co2.Sequencer)) >= 0) {
						announce.header = header
						announce.time = task.time

						// If the block is empty (header only), short circuit into the final import queue
						if header.TxHash == types.EmptyTxsHash && header.UncleHash == types.EmptyUncleHash && !common.IsSoda() {
...
func getSignerAddr(header *types.Header) (common.Address, error) {
	extraSeal := crypto.SignatureLength // Fixed number of extra-data suffix bytes reserved for signer seal
	// Retrieve the signature from the header extra-data
	if len(header.Extra) < extraSeal {
		return common.Address{}, fmt.Errorf("could not extract signature")
	}
...
func (f *BlockFetcher) importBlocks(peer string, block *types.Block) {
	...
	go func() {
		...
		if parent == nil {
			log.Debug("Unknown parent of propagated block", "difficulty", block.Difficulty(), "peer", peer, "number", block.Number(), "hash", hash, "parent", block.ParentHash())
			return
		}
