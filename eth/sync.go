...
const (
	forceSyncCycle      = 2 * time.Second // Time interval to force syncs, even if few peers are available
	defaultMinSyncPeers = 2               // Amount of peers desired to start syncing
)
...
func (h *handler) syncTransactions(p *eth.Peer) {
	...
	log.Debug("Sending transaction hashes", "count", len(hashes))
...
func (h *handler) doSync(op *chainSyncOp) error {
	...
	// This could have been located in the above function's scope but it's
	// clearer to keep it here.
	if c, ok := h.chain.Engine().(*co2.Co2); ok {
		c.SetSyncing(false)
	}
...
func (cs *chainSyncer) TriggerSync() {
	log.Info("Triggering sync called")
	op := cs.nextSyncOp()
	if op != nil {
		cs.startSync(op)
	}
