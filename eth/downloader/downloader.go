...
type Downloader struct {
	...
	notifyEndSyncFunc func() // Function to call when the sync ends
}
...
func (d *Downloader) RegisterNotifySyncEndFunc(fn func()) {
	d.notifyEndSyncFunc = fn
}
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	for i, result := range results {
		blocks[i] = types.NewBlockWithHeader(result.Header).WithBody(result.Transactions, result.Uncles).WithWithdrawals(result.Withdrawals).
			WithSodaTypes(result.Transcript, result.SequencerHeader)
	}
	// Downloaded blocks are always regarded as trusted after the
	// transition. Because the downloaded chain is guided by the
	// consensus-layer.
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		if errors.Is(err, co2.ErrSequencerBlockForValidatorNode) {
			return nil
		}
