...
func newFetchResult(header *types.Header, fastSync bool) *fetchResult {
	...
	if !header.EmptyBody() || common.IsSoda() {
...
func (q *queue) DeliverBodies(id string, txLists [][]*types.Transaction, txListHashes []common.Hash,
	...
	withdrawalLists [][]*types.Withdrawal, withdrawalListHashes []common.Hash, transcripts []*types.Transcript, sequencerHeaders []*types.Header) (int, error) {
	q.lock.Lock()
	defer q.lock.Unlock()

	validate := func(index int, header *types.Header) error {
		var sequencerBlock bool
		if common.IsSoda() {
			if len(sequencerHeaders) >= index && sequencerHeaders[index] == nil && len(*transcripts[index]) == 0 && header.Difficulty.Cmp(co2.GetDifficulty(co2.Sequencer)) == 0 {
				sequencerBlock = true
			}
			if !sequencerBlock { // We validate the block as an Executor block
				// Validate sequencer header hash is in the header
				seqHash := co2.FullSealHash(sequencerHeaders[index])
				expectedSeqHash, err := co2.RetrieveFromExtraData(shared.SequencerBlockHash, header.Extra)
				if err != nil {
					return err
				}
		...
		if header.WithdrawalsHash == nil {
			...
			// the following line was commented out since a slice like [[]] would not be nil
			// at index 0. This might be a bug, we should validate it since geth tests seem to
			// be passing and the protocol seems to be working fine.
	...
	reconstruct := func(index int, result *fetchResult) {
		...
		result.Transcript = transcripts[index]
		result.SequencerHeader = sequencerHeaders[index]
		result.SetBodyDone()
	}
...
func (q *queue) deliver(id string, taskPool map[common.Hash]*types.Header,
	...
	for _, header := range request.Headers {
		...
		if err := validate(i, header); err != nil {
			log.Error("Delivery validation failed", "err", err)
			failure = err
			break
		}
