...
func (ps *peerSet) peersWithoutTransaction(hash common.Hash) []*ethPeer {
	...
	list := make([]*ethPeer, 0, len(ps.peers))
	for _, p := range ps.peers {
		if strings.Contains(p.Name(), "executor") {
			// Exclude executors from transaction broadcasting as they do not accept
			// transactions via the Txpool. Instead Executors rely on the Sequencer
			// to sequence transactions from the Txpool as part of a block and broadcast
			// it to them.
			continue
		}
