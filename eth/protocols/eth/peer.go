...
type Peer struct {
	...
	*p2p.Peer                   // The embedded P2P package peer
	...
	executorSigCh   chan *types.ExecutorSigDetails // Channel used to queue executor signature propagation requests
	requestSigCh    chan uint64                    // Channel used to queue executor signature request
...
func NewPeer(version uint, p *p2p.Peer, rw p2p.MsgReadWriter, txpool TxPool) *Peer {
	peer := &Peer{
		...
		executorSigCh:   make(chan *types.ExecutorSigDetails),
		requestSigCh:    make(chan uint64),
	...
	go peer.dispatcher()
	go peer.sodaProtocolDispatcher()
...
// SendExecutorSignature sends an executor signature to a remote peer.
func (p *Peer) SendExecutorSignature(sigDetails *types.ExecutorSigDetails) error {
	// cast the sigDetials into an ExecutorSignaturePacket
	packet := ExecutorSignaturePacket(*sigDetails)
	return p2p.Send(p.rw, ExecutorSignatureMsg, packet)
}

func (p *Peer) SendBlockRestartRequest(blockNum uint64) error {
	return p2p.Send(p.rw, RestartBlockMsg, &RestartBlockPacket{
		BlockNumber: blockNum,
	})
}

// RequestSequencerBlock requests a sequencer block from a remote peer.
func (p *Peer) RequestSequencerBlock(blockNum uint64) error {
	return p2p.Send(p.rw, ResendSequencerBlockMsg, &ResendSequencerBlockPacket{
		BlockNumber: blockNum,
	})
}

func (p *Peer) RequestExecutorSig(blockNum uint64) error {
	return p2p.Send(p.rw, ResendExecutorSigMsg, &ResendExecutorSigPacket{
		BlockNumber: blockNum,
	})
}
...
func (p *Peer) AsyncSendExecutorSignature(sigDetails *types.ExecutorSigDetails) {
	select {
	case p.executorSigCh <- sigDetails:
	default:
		p.Log().Debug("Dropping executor signature propagation")
	}
}

func (p *Peer) AsyncRequestExecutorSignature(blockNum uint64) {
	select {
	case p.requestSigCh <- blockNum:
	default:
		p.Log().Debug("Could not request executor signature, last request not yet sent")
	}
}
