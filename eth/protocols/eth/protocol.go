...
// is primary).
var ProtocolVersions = []uint{ETH68, ETH67, SODA}

// protocolLengths are the number of implemented message corresponding to
// different protocol versions.
var protocolLengths = map[uint]uint64{ETH68: 17, ETH67: 17, SODA: 21}
...
const (
	...
	GetReceiptsMsg                = 0x0f
	ReceiptsMsg                   = 0x10
	ExecutorSignatureMsg          = 0x11
	ResendExecutorSigMsg          = 0x12
	ResendSequencerBlockMsg       = 0x13
	RestartBlockMsg               = 0x14
)
...
type RestartBlockPacket struct {
	BlockNumber uint64
}

type ResendSequencerBlockPacket struct {
	BlockNumber uint64
}

type ResendExecutorSigPacket struct {
	BlockNumber uint64
}

// ExecutorSignaturePacket is the network packet for the executor signature message.
type ExecutorSignaturePacket types.ExecutorSigDetails

func (esp *ExecutorSignaturePacket) String() string {
	return fmt.Sprintf("**ExecutorSignaturePacket** \n Signature: %x | SigHash: %x | BlockNumber: %d",
		esp.Signature, esp.SignatureHash, esp.BlockNumber)
}
...
type BlockBody struct {
	...
	Transcript      *types.Transcript    `rlp:"optional"` // Transcript contained within a block
	SequencerHeader *types.Header        `rlp:"optional"` // Sequencer header contained within a block
}

// Unpack retrieves the transactions and uncles from the range packet and returns
// them in a split flat format that's more consistent with the internal data structures.
func (p *BlockBodiesResponse) Unpack() ([][]*types.Transaction, [][]*types.Header, [][]*types.Withdrawal, []*types.Transcript, []*types.Header) {
	// TODO(matt): add support for withdrawals to fetchers
	var (
		txset            = make([][]*types.Transaction, len(*p))
		uncleset         = make([][]*types.Header, len(*p))
		withdrawalset    = make([][]*types.Withdrawal, len(*p))
		transcripts      = make([]*types.Transcript, len(*p))
		sequencerHeaders = make([]*types.Header, len(*p))
	)
	for i, body := range *p {
		txset[i], uncleset[i], withdrawalset[i], transcripts[i], sequencerHeaders[i] = body.Transactions, body.Uncles, body.Withdrawals, body.Transcript, body.SequencerHeader
	}
	return txset, uncleset, withdrawalset, transcripts, sequencerHeaders
}
...
func (*RestartBlockPacket) Name() string { return "RestartBlock" }
func (*RestartBlockPacket) Kind() byte   { return RestartBlockMsg }

func (*ResendSequencerBlockPacket) Name() string { return "ResendSequencerBlock" }
func (*ResendSequencerBlockPacket) Kind() byte   { return ResendSequencerBlockMsg }

func (*ResendExecutorSigPacket) Name() string { return "ResendExecutorSig" }
func (*ResendExecutorSigPacket) Kind() byte   { return ResendExecutorSigMsg }

func (*ExecutorSignaturePacket) Name() string { return "ExecutorSignature" }
func (*ExecutorSignaturePacket) Kind() byte   { return ExecutorSignatureMsg }
