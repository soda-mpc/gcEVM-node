...
func handleExecutorSignature(backend Backend, msg Decoder, peer *Peer) error {
	// An executor signature just arrived.
	ann := new(ExecutorSignaturePacket)
	if err := msg.Decode(ann); err != nil {
		return fmt.Errorf("%w: message %v: %v", errDecode, msg, err)
	}
	// Deliver it to the backend for queuing
	return backend.Handle(peer, ann)
}

func handleResendExecutorSignature(backend Backend, msg Decoder, peer *Peer) error {
	// A request to resend an executor signature just arrived.
	packet := new(ResendExecutorSigPacket)
...
func handleResendSequencerBlock(backend Backend, msg Decoder, peer *Peer) error {
	// A request to resend a sequencer block just arrived.
	packet := new(ResendSequencerBlockPacket)
	if err := msg.Decode(packet); err != nil {
		return fmt.Errorf("%w: message %v: %v", errDecode, msg, err)
	}
	// Deliver it to the backend for queuing
