...
func doCall(ctx context.Context, b Backend, args TransactionArgs, state *state.StateDB, header *types.Header, overrides *StateOverride, blockOverrides *BlockOverrides, timeout time.Duration, globalGasCap uint64, forEstimation bool) (*core.ExecutionResult, error) {
	...
	if forEstimation { // In gas estimation
		evm.IsGasEstimated = true
	}

	// We distinguish between a call made directly to a contract and a call made through the constructor of a contract
	// that has not yet been created.
	if args.To != nil {
		evm.IsEthCall = true
	} else {
		evm.IsEthCallWithoutDestination = true
	}
...
func DoCall(ctx context.Context, b Backend, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *StateOverride, blockOverrides *BlockOverrides, timeout time.Duration, globalGasCap uint64) (*core.ExecutionResult, error) {
	...
	return doCall(ctx, b, args, state, header, overrides, blockOverrides, timeout, globalGasCap, false)
}
...
func executeEstimate(ctx context.Context, b Backend, args TransactionArgs, state *state.StateDB, header *types.Header, gasCap uint64, gasLimit uint64) (bool, *core.ExecutionResult, error) {
	...
	result, err := doCall(ctx, b, args, state, header, nil, nil, 0, gasCap, true)
	if err != nil {
